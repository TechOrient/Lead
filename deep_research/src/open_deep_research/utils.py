...
##########################
# Tavily Search & SerpAPI Tools Tool Utils
##########################
...
SERP_API_DESCRIPTION = (
    "A search engine just only user&company page from LinkedIn"
    "This descr. makes LinkedIn search for people"
)
"""
@tool(description=SERP_API_DESCRIPTION)
async def serp_linkedin(
    company_name : str,
    ...
    if not api_key:
        return "SERP API API KEY BULUNAMADI"
    ...
    for all_result in organic_results:
...
def linkedin_search(company_name:str,api_key=os.getenv("SERP_API"),company_address : Optional[str]= None):

    query = f"{company_name} site:linkedin.com/company"
    if company_address:
        query += f" {company_address}"
    ...
    if not organic_results:
        return f"{company_name} için LinkedIn profili bulunamadı."
    ...
    return "\n".join(search_results) if search_results else f"{company_name} için LinkedIn profili bulunamadı."
